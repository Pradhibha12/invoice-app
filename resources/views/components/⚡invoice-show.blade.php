...
new class extends Component
{
    ...
    public $paymentType = 'full'; // 'full' or 'partial'
    public $paymentAmount = 0.00;
    public $paymentMode = 'UPI';
    public $paidOn = '';
    ...
    {
        $remaining = $this->invoice->total - $this->invoice->amount_paid;
        $this->paymentAmount = max(0, round($remaining, 2));
        $this->paymentType = 'full';
        $this->paidOn = now()->format('Y-m-d');
        // default default payment mode if invoice has one
        $this->paymentMode = $this->invoice->payment_mode ?? 'UPI';
    }
    ...
    public function recordPayment()
    {
        ...
        $this->validate([
            'paymentAmount' => 'required|numeric|min:0.01|max:' . round($remaining, 2),
            'paymentMode' => 'required|in:Cash,Bank Transfer,UPI,Cheque,Card,Other',
...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    @if ($showPaymentForm)
        <div class="bg-white p-6 rounded-xl border border-stone-200 shadow-sm space-y-4">
            <h3 class="text-lg font-bold text-stone-900 font-display">Record Payment</h3>
            <form wire:submit="recordPayment" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    <label class="block text-xs font-semibold text-stone-500 uppercase">Payment Type</label>
                    <select wire:model.live="paymentType" class="mt-1 block w-full rounded-lg border-stone-300 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-200 focus:ring-opacity-50 text-sm">
                ...
                <div>
                    <label class="block text-xs font-semibold text-stone-500 uppercase">Amount (₹)</label>
                    <input wire:model="paymentAmount" type="number" step="0.01" min="0.01" {{ $paymentType === 'full' ? 'readonly' : '' }} class="mt-1 block w-full rounded-lg border-stone-300 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-200 focus:ring-opacity-50 text-sm bg-stone-50 readonly:bg-stone-100">
                    @error('paymentAmount') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-xs font-semibold text-stone-500 uppercase">Payment Mode</label>
                    <select wire:model="paymentMode" class="mt-1 block w-full rounded-lg border-stone-300 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-200 focus:ring-opacity-50 text-sm">
                    ...
                    @error('paymentMode') <span class="text-red-500 text-xs block mt-1">{{ $message }}</span> @enderror
                </div>

                <div>
                    <button type="submit" class="w-full px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold shadow-sm transition">
                        Submit Payment
                    </button>
                </div>
            </form>
