...
new class extends Component
{
    public function getStats()
    {
        $user = auth()->user();
        if ($user->role === 'client') {
            $totalRevenue = Invoice::where('client_id', $user->client_id)->sum('amount_paid');
            $totalOutstanding = Invoice::where('client_id', $user->client_id)
                ->where('status', '!=', 'paid')
                ->get()
    ...
    public function recentInvoices()
    {
        $user = auth()->user();
        if ($user->role === 'client') {
            return Invoice::where('client_id', $user->client_id)
                ->latest()
                ->get();
        }
...
@if(auth()->user()->role === 'client')
    <div class="space-y-8">
        ...
        <div class="bg-gradient-to-r from-teal-800 to-teal-700 rounded-2xl p-6 text-white shadow-md border border-teal-900/10">
            <h1 class="text-3xl font-extrabold tracking-tight font-display mb-2">Welcome, {{ auth()->user()->name }}</h1>
            <p class="text-teal-100/90 max-w-xl">Review your invoices, balances, and payment statuses directly from your client portal.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Paid Total Card -->
            <div class="bg-white p-6 rounded-xl border border-stone-200 shadow-sm flex items-center space-x-4">
                ...
                <div>
                    <span class="text-sm font-semibold text-stone-500 uppercase tracking-wider block">Total Paid</span>
            ...
            <div class="bg-white p-6 rounded-xl border border-stone-200 shadow-sm flex items-center space-x-4">
                ...
                <div>
                    <span class="text-sm font-semibold text-stone-500 uppercase tracking-wider block">Pending Balance</span>
        ...
        <div class="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-stone-100">
                <h2 class="text-lg font-bold text-stone-950 font-display">Your Invoices</h2>
            </div>
                ...
                @forelse($this->recentInvoices() as $invoice)
                    <div class="px-6 py-4 flex items-center justify-between hover:bg-stone-50/50 transition">
                        <div class="flex items-center space-x-4">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm text-white bg-amber-600">
                                IN
                            </div>
                            <div>
                                ...
                                <span class="text-xs text-stone-500 block">Due on {{ $invoice->due_date }}</span>
                            </div>
                        </div>
